...
class create_episodes(data.Dataset):
    def __init__(self,root,path_to_folder,n_shot=5,n_eval=15,transform=None)
    """Args:
        ...
        """
        root=os.path.join(root,path_to_folder)
        self.labels=
        images=
        self.episode_loader=[data.DataLoader(ImageDataset(images=images[idx],label=idx,transform=transform),
...
def prepare_data():
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
     train_set = create_episodes(args.data_root, 'train', args.n_shot, args.n_eval,
        transform=transforms.Compose([
            ...
            transforms.ToTensor(),
            normalize]))
    test_set = create_episodes(args.data_root, 'test', args.n_shot, args.n_eval,
    transform=transforms.Compose([
        transforms.Resize(args.image_size * 8 // 7),
            transforms.CenterCrop(args.image_size),
            transforms.ToTensor(),
            normalize]))
    val_set = create_episodes(args.data_root, 'val', args.n_shot, args.n_eval,
    transform=transforms.Compose([
        transforms.Resize(args.image_size * 8 // 7),
            transforms.CenterCrop(args.image_size),
            transforms.ToTensor(),
